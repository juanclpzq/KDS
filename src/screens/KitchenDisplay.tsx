 ...
 * Main view showing all active orders in a grid
 */
export function KitchenDisplay() {
  ...
  useEffect(() => {
    ...
    // Update every second to refresh elapsed times
  ...
  return (
    <div className="min-h-screen bg-kds-bg p-6">
      {/* Header */}
        ...
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <p className="text-gray-500">
              Orders will appear here when they are paid
            </p>
          </div>
        </div>
      )}
      ...
      <footer className="fixed bottom-2 right-2 text-xs text-gray-600">
        <div className="bg-gray-900 bg-opacity-80 px-3 py-1 rounded">
          KDS v1.0 | {new Date().toLocaleTimeString()}
        </div>
      </footer>
    </div>
  );
